...
resource "azurerm_subnet_network_security_group_association" "nsg-associate" {
  ...
  network_security_group_id = count.index <= length(azurerm_network_security_group.nsg-defined) ? azurerm_network_security_group.nsg-defined[count.index].id : azurerm_network_security_group.nsg-default[0].id
}